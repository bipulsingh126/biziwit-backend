...
router.post('/', async (req, res, next) => {
  try {
    ...
    // Ensure slug is sanitized and URL-safe without spaces
    reportData.slug = slugifyReportUrl(reportData.slug, reportData.title);
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // Slug handling: Prioritize sanitized Excel slug, then generate from title, then fallback.
        let slug = slugifyReportUrl(cleanUrlSlug, cleanTitle);
        if (!slug) {
          try {
            slug = await generateUniqueSlug(cleanTitle);
          } catch (slugError) {
