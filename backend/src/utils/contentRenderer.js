...
export function renderMarketShareGainPage(servicePage) {
  ...
  html += `</header><main>`;
  html += `<section><h2>What Market Share Growth Really Delivers</h2>`;
  ...
  html += `</ul>`;
  html += `<p>Organizations that consistently gain share outperform competitors not only in revenue growth but in long-term profitability and resilience.</p></section>`;

  html += `<section><h2>Growth Pillars That Win Markets</h2><ul>`;
  html += `<li><h3>Competitor Analysis</h3><p>Systematic mapping of competitor positioning, pricing architecture, distribution strength, and share concentration.</p></li>`;
  html += `<li><h3>Market Expansion Strategy</h3><p>Identification and prioritization of high-potential geographies, segments, and product adjacencies.</p></li>`;
  html += `<li><h3>Branding &amp; Social Media</h3><p>Sharper positioning frameworks and integrated online-offline activation strategies.</p></li>`;
  html += `<li><h3>Channel &amp; Distribution Optimization</h3><p>Channel mix evaluation across digital, retail, distributor, and partner networks.</p></li>`;
  html += `</ul></section>`;

  html += `<section><h2>The 5P Framework for Market Share Gain</h2>`;
  ...
  html += `<section><h2>How We Create Competitive Advantage</h2><ul>`;
  html += `<li><h3>Sharper Competitive Positioning</h3><p>Move beyond reactive competition. Define a differentiated stance that protects margins and reduces price-led erosion.</p></li>`;
  html += `<li><h3>Prioritized Growth Opportunities</h3><p>Stop spreading resources thin. Focus on high-impact geographies, segments, and channels with proven expansion potential.</p></li>`;
  html += `<li><h3>Stronger Market Penetration</h3><p>Convert strategy into visibility and presence. Deploy optimized online and offline channels that increase reach and conversion efficiency.</p></li>`;
  html += `<li><h3>Measurable, Sustainable Share Gains</h3><p>Avoid short-term spikes driven by discounts. Build structural growth backed by continuous performance tracking and strategic refinement.</p></li>`;
  html += `</ul></section>`;
...
export function renderFtePage(servicePage) {
  ...
  html += `<section><h2>What is the FTE Model?</h2>`;
  ...
  html += `<ul>`;
  html += `<li><strong>Dedicated Resources:</strong> Exclusively committed analysts working directly for your organization.</li>`;
  html += `<li><strong>Team Integration:</strong> Seamless collaboration using your communication tools, templates, and workflows.</li>`;
  html += `<li><strong>Flexible Engagement:</strong> Scalable model with resource swapping as project requirements shift.</li>`;
  html += `<li><strong>Cost Efficiency:</strong> Significantly lower total cost of ownership compared to traditional consulting or in-house hiring.</li>`;
  html += `<li><strong>Continuous Support:</strong> Dedicated analyst availability aligned with your time zone and operational tempo.</li>`;
  html += `<li><strong>Faster Turnaround:</strong> Onboarding in less than 5 days with accelerated delivery schedules.</li>`;
  html += `</ul></section>`;

  html += `<section><h2>FTE Services We Offer</h2><ul>`;
  html += `<li><h3>Market Research Analysts</h3><p>Dedicated support for secondary/primary research, database building, opportunity sizing, and competitive tracking.</p></li>`;
  html += `<li><h3>Data Analysts</h3><p>Advanced Excel, Python, Power BI, and Tableau-based quantitative modeling and insights generation.</p></li>`;
  html += `<li><h3>Consulting Support Staff</h3><p>Business problem-solving, client presentation decks, strategic benchmarking, and market intelligence.</p></li>`;
  html += `<li><h3>Industry SMEs (On-Demand)</h3><p>Plug-in senior domain advisors for strategic input, expert validation, and high-impact report generation.</p></li>`;
  html += `</ul></section>`;

  html += `<section><h2>Why Choose Bizwit for FTE Support</h2>`;
  html += `<p>At Bizwit, our dedicated resources operate as a seamless extension of your internal team, bringing structured research methodologies, analytical rigour, and consulting discipline to every engagement.</p>`;
  html += `<ul>`;
  ...
  html += `<section><h2>How It Works (5-Step Deployment Process)</h2><ol>`;
  html += `<li><strong>Define Scope &amp; Skillset:</strong> We collaborate closely with your team to clearly define project objectives, required skillsets, experience levels, and desired outcomes.</li>`;
  html += `<li><strong>Select Profiles:</strong> We shortlist and present candidate profiles from our talent pool for your review and selection.</li>`;
  html += `<li><strong>Trial Phase (Optional):</strong> Assess selected resources in real-time project environments with a risk-free trial period.</li>`;
  html += `<li><strong>Full Onboarding:</strong> Complete documentation, compliance, system access, and knowledge transfer for seamless Day 1 productivity.</li>`;
  html += `<li><strong>Monthly Reviews &amp; Adjustments:</strong> Continuous collaboration through monthly performance reviews, feedback loops, and resource optimization.</li>`;
  html += `</ol></section>`;

  html += `<section><h2>Success Metrics &amp; Proven Impact</h2><ul>`;
  html += `<li><strong>60% Faster Project Delivery:</strong> Streamlined delivery pipelines for complex research tasks.</li>`;
  html += `<li><strong>60+ FTEs Deployed:</strong> Experienced analysts deployed across 7 global countries.</li>`;
  html += `<li><strong>2x Output Improvement:</strong> Enhanced long-term productivity and research output.</li>`;
...
export function renderThoughtLeadershipPage(servicePage) {
  ...
  html += `<p>70–80% of B2B buyers prefer engaging with insightful industry content before speaking with a vendor. Organisations that consistently share expert perspectives often influence buying decisions long before formal sales conversations begin.</p>`;
  html += `<p>Recent industry surveys of over 15,000 marketing leaders (from VPs to CXOs) show that 83% believe strong positioning in the new economy is critical for driving B2B growth.</p></section>`;

  html += `<section><h2>What We Offer</h2><ul>`;
  html += `<li><h3>Asset Creation &amp; Optimization</h3><p>Whitepapers, eBooks, blog series, infographics, SEO-optimized content, and gated resources for qualified lead generation.</p></li>`;
  ...
  html += `<li><h3>Distribution &amp; Lead Capture</h3><p>Targeted email campaigns, LinkedIn strategies, media partnerships, lead capture mechanisms, and performance monitoring.</p></li>`;
  html += `<li><h3>Performance &amp; ROI Optimization</h3><p>Lead scoring, pipeline velocity tracking, content performance analytics, and continuous conversion rate optimization.</p></li>`;
  html += `</ul></section>`;
...
export function renderMarketIntelligencePage(servicePage) {
  ...
  html += `<p><strong>At the Feasibility Stage:</strong> Businesses evaluate market size, demand potential, regulatory considerations, and competitive intensity to determine whether an opportunity is worth pursuing, reducing uncertainty before committing resources.</p>`;
  html += `<p><strong>During Market Entry:</strong> Intelligence supports decisions around target segments, value positioning, partnership models, and go-to-market strategies, enabling organisations to enter new markets with a clear competitive advantage.</p>`;
  html += `<p><strong>As Companies Scale:</strong> Expansion-focused intelligence identifies untapped customer segments, regional growth opportunities, evolving demand patterns, and emerging competitive threats.</p></section>`;
...
export function renderEsgConsultingPage(servicePage) {
  ...
  html += `<li><h3>ESG Strategy &amp; Consulting</h3><p>Develop tailored Environmental, Social, and Governance (ESG) strategies aligned with investor expectations, GRI, SASB, TCFD, CSRD, and regulatory frameworks.</p></li>`;
  ...
  html += `<li><h3>Environmental Impact Reporting</h3><p>Comprehensive life-cycle assessments (LCA) and environmental data insights for product and corporate transparency.</p></li>`;
  html += `</ul></section>`;
...
export function renderIndiaGtmStrategyPage(servicePage) {
  ...
  html += `<li><strong>Economic Leadership:</strong> India is on track to become the 3rd largest economy globally.</li>`;
  html += `</ul></section>`;
...
export function renderVocPage(servicePage) {
  ...
  html += `<section><h2>Why Voice of Customer Matters</h2>`;
  html += `<p>Understanding what customers truly think, need, and experience is critical to driving product innovation, reducing churn, and increasing lifetime value. Our VOC frameworks gather deep primary feedback across touchpoints.</p></section>`;

  html += `<section><h2>Our VOC Capabilities</h2><ul>`;
  html += `<li><h3>Customer Journey &amp; Touchpoint Mapping</h3><p>Trace the end-to-end customer lifecycle to identify friction points and moments of delight.</p></li>`;
  html += `<li><h3>Sentiment &amp; Perception Analytics</h3><p>Extract qualitative and quantitative insights from customer conversations, reviews, and feedback.</p></li>`;
  html += `<li><h3>NPS &amp; Satisfaction Benchmarking</h3><p>Measure Net Promoter Score, CSAT, and Customer Effort Score against industry peers.</p></li>`;
  html += `<li><h3>Product Feedback &amp; Feature Prioritization</h3><p>Direct user input into your product and service roadmaps.</p></li>`;
  html += `</ul></section>`;
